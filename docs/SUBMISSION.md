...
4. **Offline Evaluation & Validation:**
   ...
   - `starter/train.ts` trains on three practice scenarios and reports `Baseline: Practice 0 / Held-out 0` → `Trained: Practice 12 / Held-out 12` (4/4 on each of three held-out variants: different seed/flood, swapped start, and long flood), demonstrating that the MLP generalizes beyond the scenarios it was trained on.

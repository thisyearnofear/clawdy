...
| Mint, Tripo, Convex integrations | `mintdotgg/mint-threejs-skills` is installed, Mint rover GLB is loaded. **Convex Season 0 sync shipped:** `convex/` schema for checkpoints, examples, match summaries, and training jobs; dual-write from Coach/Play via guest keys; Coach UI lineage strip. LocalStorage remains the offline source of truth when `NEXT_PUBLIC_CONVEX_URL` is unset. | Add Convex Auth; optional replay blob storage; cloud restore into the workbench. |

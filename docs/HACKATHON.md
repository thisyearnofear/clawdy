...
**September 18 addendum:** the owner approved a mesh-first playable ground, superseding the original generated-world requirement for the playable course. The active terrain is the Blender-authored `public/terrain/sandstone-basin.glb` used identically for rendering and collision; the Marble world pipeline was retired and removed on September 26, 2026 (git history holds it); the Mint terrain candidate remains in the registry but is not active.
...
- The claim sentence is itself CI-enforced: the `eval:gate` claims block recomputes from raw match data every run — abstract per-leg parity within 1 banked of the safe teacher plus an identical champion-win set, and a physics no-collapse floor of floor(safe/2) per leg. Product docs may never claim more than the gate enforces; a failing claim is reported, not re-pinned away.
- **Measured status (Sep 26, distill `2b76a6113d66`, 161 examples):** on unseen abstract layouts the trained student matches the teacher's outcomes — 8/8 legs within 1 banked, the same 5 champion wins, totals 38 vs 40 — and beats the untrained baseline 4/4 held-out boards (+25 banked, starter CLI). On physics courses it still trails the per-tick Dijkstra teacher (grounded 31 vs 35, family 63 vs 68 — gaps narrowed from −8/−13 to −4/−5 on Sep 26 by the physics-aware rollout oracle; every leg at or above the half-of-safe floor). The gap is disclosed, not hidden: "beats what it was never taught" is claimed only against the untrained baseline and the rival, with teacher parity — never teacher superiority — on the abstract held-out set.
...
| --- | --- | --- |
| World Labs | Generate the environment used for practice and matches. | Superseded September 18: the owner approved a mesh-first playable ground, so the active terrain is the Blender-authored `public/terrain/sandstone-basin.glb` used identically for rendering and collision. The Marble world pipeline was retired and removed on September 26, 2026 (git history holds the generated-world provenance); the Mint terrain candidate remains in the registry but is not active. |
...
**Play-first Season 0 shell:** Practice/Match modes, collapse-able Coach panel, path/landmark readability overlays, and scored-match coaching lock are wired in `ArenaScene` / `ArenaWorldView`. Starter CLI trains on the six practice builder scenarios and evaluates on held-out variants. Since September 18 the active ground is the mesh-first Sandstone Basin terrain (Blender-authored local GLB, included — explicit rebuild `blender --background --factory-startup --python scripts/build-arena-terrain.py -- --force`); the earlier generated-world rebuild strategy was retired and removed on September 26, 2026 (git history holds it).
...
| Versioned course | `services/arenaCourse.ts` grounds Course 01 against the included Sandstone Basin terrain GLB (SHA pinned, shared by render and collider via `services/arenaTerrain.ts`). `applyCourseMode` switches Practice vs Match flood/resource layouts on the same world. | Browser round-trip QA done Sep 26 (workbench loop passes in a real browser). |
| Baselines and learned policy | `services/arenaPolicy.ts` supports safe, greedy, weather, and `learned` (`PolicyCheckpoint`) strategies. The learned policy runs through `createLearnedPolicy` (checkpoint v3, residual edge-pointer head). Teachers and student share one executor (`services/arenaControllerRules.ts`) — no student-only scaffolding. Current gate (`2b76a6113d66`, 161 examples): abstract dual-side trained **38** vs safe **40** with 8/8 legs within 1 and an identical champion-win set (5 = 5); grounded trained **31** vs safe **35**; family **63** vs **68**, every leg above the harness-enforced half-of-safe floor. `services/courseFamily.ts` generates 4 validated layouts; `npm run eval:gate` pins 16 abstract + 8 grounded + 16 family legs with side-swaps in `docs/eval-gate.json` and enforces the claims block from raw match data. Consequence rollouts are physics-aware on grounded boards (`rolloutOutcomeDelta` takes an optional motion adapter; `restoreSnapshot` re-seeds it); edge-only argmax mining is re-enabled on the grounded practice board (phase 3d, cap 6). | Remaining transfer gap to the teacher: −4 grounded / −5 family. Next levers: raise the grounded argmax cap if new labels keep measuring ≥ +1.0 under physics; deeper adversarial family coverage. |
...
| Active application UI | Play-first workbench: Play/Pause/Replay/Coach, Practice↔Match toggle, path ribbons + landmarks, rival silhouette, Coach collapsed by default (locked on Match). **Sep 26:** Help drawer + Next-step chip; champion name/look; Coach specialize chips + post-train focus line; focus fingerprint bars on field cards; proximity **CLASH** overlay (`arenaEncounter` → cargo steal / stagger). Boot stages, world-ready veil, mode flash, flood tip. Replay cinematic + tournament bracket vs the house field; opt-in Record clip; share/print on round complete. Convex lineage strip when configured. | Mobile polish; richer encounter types; Mint-auth distinct rival GLB if regenerating. |
| World loading / visuals | Single pinned Blender terrain GLB (`/terrain/sandstone-basin.glb`, SHA-256 enforced) rendered directly in both lite and full modes; flat route ribbons, low landmarks, warm basin lighting. The Marble splat/HQ assets were retired and removed September 26, 2026. | Visual pass and low-end device checks when browser scope resumes. |
| Checkpoint format and storage | `policyModel.ts` defines a 36 → 32 → 16 → 8 MLP with Tanh/softmax plus a v3 linear edge-pointer head, validated as `season-0.checkpoint.v3` JSON (v1/v2 metadata-readable, execution-quarantined). `checkpointStorage.ts` handles `localStorage`, JSON import/export, and validation. The weight identity is a deterministic digest, not a cryptographic SHA-256. | Implement a real SHA-256 if the docs must claim it. |
| Training runtime / starter | `policyTrainer.ts` implements supervised cross-entropy backpropagation with momentum SGD plus the v3 edge-softmax term. `starter/train.ts` runs `npm run starter:train` end-to-end and exports `starter/champion-checkpoint.json` (re-minted under v3). It trains on the six practice scenarios and reports `Practice 48 banked / 6 wins` and held-out `+25` banked over the untrained base with 4/4 wins. | Keep the starter artifact re-minted with every training-path version bump (done Sep 26). |
| Mint, Tripo, Convex integrations | `mintdotgg/mint-threejs-skills` is installed, Mint rover GLB is loaded. **Convex is the system of record:** project `clawdy` on Convex cloud; prod URL on Vercel (`NEXT_PUBLIC_CONVEX_URL`). `convex/sync.ts` is the only sync surface — typed payload validators, LWW reject + server-clock ordering, tombstone deletes, write-side caps; the client (`services/syncEngine.ts` + zustand `arenaStore`) keeps localStorage as a write-behind cache and parks offline writes in a persisted outbox with a status chip. Coach lineage strip; guest-key exhibition trust (single `resolveOwner` seam for a later Convex Auth swap). With Convex unset everything stays local + JSON export. | Add Convex Auth; optional replay blob storage. |

### Implemented Reference Contract

The simulation uses `season-0.reference.2`; the checkpoint artifact uses `season-0.checkpoint.v3`. Neither is a frozen competition release. It uses 50 ms ticks and a decision every five ticks. Course `sandstone-basin-course-1` runs for 1200 ticks (60 seconds), with twelve cores, thirteen stations, twenty-six connections (ten floodable), three scheduled flood windows in practice and two in match. Flooding applies a route-speed penalty; the water overlay is not a fluid simulation.
...
The learned policy is a 2-layer MLP: 36 normalized observation features, hidden layers of 32 and 16 with Tanh, and an 8-class action head with softmax. Inference selects the available action whose class has the highest logit; for move classes, checkpoint v3 ranks the same-class candidate roads with a linear edge-pointer head applied as a residual over the shared executor heuristic (zero-init ⇒ exact legacy ordering). Training is supervised cross-entropy on approved `ArenaTrainingExample` tuples with momentum SGD, plus the edge-head cross-entropy term when a move label carries an `edgeId`; it writes a new `PolicyCheckpoint` with a parent reference, training summary, and a deterministic weight digest. Checkpoints are validated for shape, finite values, and schema version before use. `ArenaSession` loads the checkpoint into the runner, and the UI persists checkpoints/examples in `localStorage` with JSON import/export.
...
- `npm test` — 22 test files / 183 tests passed (includes `championIdentity`, `arenaEncounter`, coaching focus vectors, the Convex `sync.test.ts` suite via convex-test, v3 edge-head pins, browser few-shot train stability, and the rollout-physics restore-fidelity pins).
- `npm run eval:gate` — green against pin `2b76a6113d66` (161-example distill, physics-aware grounded labels): 16/16 abstract + 8/8 grounded + 16/16 family legs reproduce the pin, hard floors hold, the claims block passes (8/8 abstract parity, identical 5-leg win set, 12/12 physics legs at or above the half-of-safe floor), frames 23/23 (passing-subset re-pin; `builder-course-04-early-flood-t80` dropped as a behavior guard, not a claim).
- `npm run lint` / `npx tsc --noEmit` / `npm run build` expected green on the intended `main` commit; re-run before submission.
- `npm run starter:train` runs end-to-end and exports `starter/champion-checkpoint.json`.
- Live exhibition: [https://clawdy-nine.vercel.app/](https://clawdy-nine.vercel.app/) with Convex prod URL when env is set.
...
- Encoding/training tests verify the 36-dimensional vector, MLP forward pass, edge-feature encoding, checkpoint validation, cross-entropy training, and the `proposeCorrection` coaching engine.
- Encounter tests cover proximity gating, weather-vs-greedy clash resolution, and fingerprint lines from approved examples.
...
- The server-rendered entrypoint test verifies the loading shell (Play → Replay → Coach), Help control, and training-status disclosure without wallet onboarding.
...
- Production presentation-device QA and the recorded two-minute walkthrough remain open on [SUBMISSION_CHECKLIST.md](SUBMISSION_CHECKLIST.md).

### Remaining Foundation and Retirement Work

- ~~Verify the actual terrain view, rover visibility, camera framing, replay-cam cinematic behavior, tournament bracket/watch flow, loading/retry, controls, and layout once browser testing is approved.~~ Done Sep 26: full-workbench browser QA passed (boot → Play → review → coach → train → bracket → offline readout → quarantine → Match lock → Help a11y).
...
- ~~Verify the `ArenaScene` held-out guard works in a browser once visual testing is approved.~~ Done Sep 26: Match mode shows the "Coaching locked." chip and disables the Coach button.
- September 26, 2026 retirement pass: verified the wallet-era code was already gone — `CloudScene`, the old physics hook, wallet configuration, queue modules, legacy `AgentProtocol`, API routes, and contract/indexer tooling no longer exist on disk, and CI carries no chain settings. Removed what actually remained: the Marble pipeline (`services/marbleWorld.ts` + its test, the generate/rebuild scripts, tracked `public/marble/` assets, `docs/MARBLE_STARTER_KIT.md`, the Marble env block in `.env.example`), the legacy 0g/WalletConnect env examples, `services/logger.ts`, the unused `@react-three/rapier` dependency, and the foundry `.gitignore` block. `services/arenaTransport.ts` is kept deliberately as the documented future `decision → ack → action_result` transport seam (see `docs/INSPIRATION.md`). No external service was shut down.

...
- **Measured status (Sep 26, distill `9c1e938a3d6b`, 162 examples):** on unseen abstract layouts the trained student matches the teacher's outcomes — 8/8 legs within 1 banked, the same 5 champion wins, totals 38 vs 40 — and beats the untrained baseline 4/4 held-out boards (+25 banked, starter CLI). On physics courses it still trails the per-tick Dijkstra teacher (grounded 27 vs 35, family 55 vs 66; every leg at or above the half-of-safe floor). The gap is disclosed, not hidden: "beats what it was never taught" is claimed only against the untrained baseline and the rival, with teacher parity — never teacher superiority — on the abstract held-out set.
...
| Baselines and learned policy | `services/arenaPolicy.ts` supports safe, greedy, weather, and `learned` (`PolicyCheckpoint`) strategies. The learned policy runs through `createLearnedPolicy` (checkpoint v3, residual edge-pointer head). Teachers and student share one executor (`services/arenaControllerRules.ts`) — no student-only scaffolding. Current gate (`9c1e938a3d6b`, 162 examples): abstract dual-side trained **38** vs safe **40** with 8/8 legs within 1 and an identical champion-win set (5 = 5); grounded trained **27** vs safe **35**; family **55** vs **66**, every leg above the harness-enforced half-of-safe floor. `services/courseFamily.ts` generates 4 validated layouts; `npm run eval:gate` pins 16 abstract + 8 grounded + 16 family legs with side-swaps in `docs/eval-gate.json` and enforces the claims block from raw match data. | Physics-transfer gap to the teacher (route-only consequence rollouts mis-price travel physics); a physics-aware rollout oracle would let the edge-only argmax mining (cap 0, machinery in-tree) re-enable. |
...
- `npm test` — 20 test files / 172 tests passed (includes `championIdentity`, `arenaEncounter`, coaching focus vectors, the Convex `sync.test.ts` suite via convex-test, v3 edge-head pins, and browser few-shot train stability).
- `npm run eval:gate` — green against pin `9c1e938a3d6b` (162-example distill): 16/16 abstract + 8/8 grounded + 16/16 family legs reproduce the pin, hard floors hold, the claims block passes (8/8 abstract parity, identical 5-leg win set, 12/12 physics legs at or above the half-of-safe floor), frames 24/24.

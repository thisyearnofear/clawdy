...
| Baselines and learned policy | `services/arenaPolicy.ts` supports safe, greedy, weather, and `learned` (`PolicyCheckpoint`) strategies. The learned MLP runs through `createLearnedPolicy`. Oracle-routed consequence distill (169 examples, pin `122f676a3d4f`): grounded trained 33 vs safe 36; abstract dual-side trained 36 vs safe 44; family 43 vs 72. `services/courseFamily.ts` generates 4 validated layouts; `npm run eval:gate` pins 16 abstract + 8 grounded + 16 family legs with side-swaps in `docs/eval-gate.json`. | Close compete open-loop gap (9 vs safe 12) via base-policy grounded mining; close family gap; human-coached training at scale. |

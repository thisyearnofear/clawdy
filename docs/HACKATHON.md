...
| --- | --- | --- |
| World Labs | Generate the environment used for practice and matches. | An agent traverses a recognizable generated world with validated collision and routes. The HQ textured mesh (~600k triangles) is exported and loaded from the World Labs CDN as the primary terrain visual. |
...
| Physical controller | `services/arenaPhysics.ts` uses Rapier 0.19.2 with a kinematic rigid body that terrain-follows via downward ray casts, computes pitch/roll from surface normals, blocks on walls via horizontal ray casts, and outputs a full quaternion. Blocked movement triggers a recorded recovery. All seven edges pass traversal tests in both directions with zero recoveries. | Further adversarial course coverage and deeper mobile physics testing. This is not wheeled vehicle dynamics. |
| Versioned course | `services/arenaCourse.ts` grounds six stations and seven connections against the committed collider. Its loader checks the collider SHA-256 and rejects mismatches. All seven edges pass traversal tests in both directions. The HQ textured mesh (~600k triangles) is loaded from the World Labs CDN as the primary terrain visual, with the splat as fallback. | Validate a second course/collider version and Mint-generated world colliders when available. Course 01 is public practice, not a held-out evaluation set. |

...
| **Heygears** | 3D printing hardware (UltraCraft, Reflex series) + AI design software ("AI Box") | Stretch only. We can export the champion rover GLB to STL with printer-profile metadata and ship it as a downloadable "gift artifact" inside the submission. The "world as a gift" theme lands literally if judges can 3D-print the rover. | High for the STL export plus README metadata (about 1 day). A full printed-physical-piece submission would need an offline print run, which we are not committing to. |
...
- [x] Sep 24 pin (`122f676a3d4f`): timing-first syllabus + anti-oscillation / bank-at-base executor — grounded trained 33 (was 29), abstract dual-side 36 (was 20), heldout-02 normal 5 (was 0). Compete open-loop gap (9 vs safe 12) diagnosed via safe-trajectory shadow; next lever is base-policy grounded mining for post-bank flood routing (see `docs/COMPATIBILITY.md`).

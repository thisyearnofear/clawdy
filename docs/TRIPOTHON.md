...
- [x] Souvenir clip export (Sep 24): every non-lite run auto-records the canvas to WebM (MediaRecorder, coarse-pointer gated off); "Save clip" download button appears on the match-complete panel. Verified live (`clawdy-sandstone-practice-01-1200.webm` offered at full time).

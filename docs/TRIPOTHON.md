...
| 3 | Spark coach-trail overlay | **World Labs Spark** (in live use in `components/environment/BankBursts.tsx`; the old `MarbleWorldLayer.tsx` was removed in the Sep-26 cleanup) | A new `CoachTrailLayer` component inside `<World>`. Reads the `currentMistake.suggested` edgeId from `ArenaScene.tsx` and emits a thin colored splat strip along `course.scenario.edges[edgeId].path`. | 1–2 days | High (World Labs judge sees their renderer doing something new; a coach ghost-path is a unique overlay) |
...
- **Spark** is built by World Labs and is already imported by `BankBursts.tsx` (lazy WASM import). Its Dyno stdlib and procedural splat APIs are documented on sparkjs.dev. The coach trail is a small additional `SplatMesh` placed alongside the world splat, no new dependencies.
...
| 4 | Sep 20 | Capture coaching loop + trained-checkpoint replay in 1080p desktop. **Pick 3 prototype**: read Spark Dyno overview, place a secondary `SplatMesh` inside the world canvas (mount like `BankBursts.tsx` — `MarbleWorldLayer` has since been retired) and confirm it renders. |
...
- [x] Guidance + identity (Sep 26): Help drawer, next-step chip, specialize chips, champion name/look.
- [x] Roadmap + encounter slice (Sep 26): [ROADMAP.md](ROADMAP.md); focus fingerprint; proximity clash resolves from specialization vectors (cargo steal / stagger).

...
export class EconomyEngine {
  ...
  collectAsset(session: AgentSession, stats: MemeAssetStats, finalRushMultiplier: number = 1): { earned: number; vitalityGain: number; burdenGain: number } {
    ...
    switch (stats.type) {
      case 'air_bubble':
        session.airBubbleUntil = Date.now() + 6000
        session.airBubbleCount = (session.airBubbleCount ?? 0) + 1
        earned = 0.0005; vitalityGain = 0; burdenGain = -8
        break
      case 'foam_board':
        session.foamBoardUntil = Date.now() + 9000
        session.foamBoardCount = (session.foamBoardCount ?? 0) + 1
        earned = 0.001; vitalityGain = 2; burdenGain = -10
        break
      case 'drain_plug':
        session.drainPlugCount = (session.drainPlugCount ?? 0) + 1
        earned = 0.001; vitalityGain = 0; burdenGain = -12
        break
      case 'golden_meatball':
        earned = 0.01; vitalityGain = 25; burdenGain = -15
        break
      case 'spicy_pepper':
        session.speedBoostUntil = Date.now() + 10000
        vitalityGain = 5; burdenGain = 10
        break

...
1. Write an asset plan. Prefer discrete models, coherent asset packs,
   ...
   scaffolding UI.
2. Read `mint-project-workspaces.md`. Resolve or create one Mint Project for the
   local codebase, persist it in `mint-assets.json`, and use that Project for
   every chat and asset created for the work.
3. Confirm the live Mint MCP capabilities. Do not invent tool names, schemas,
   lifecycle stages, or downloadable formats.
4. Start generation in the default automatic mode. Pass the resolved
   `project_id` whenever `chat_id` is absent. Use review mode only when the user
   explicitly asks to inspect, compare, revise, select, or approve a preview.
5. Follow returned `nextSteps` until the asset reaches final status. Prefer the
...
6. For models, packs, materials, Images, animation, and audio, use the
...
7. Generate a world only when the user explicitly requests a generated world
...
8. Read `asset-pipeline.md`. Save the manifest temporarily and run
...
9. Before loading a GLB, read `gltf-runtime-compatibility.md`, inspect the
   ...
   separate approval.
10. Show the returned `chatUrl` as the Mint handoff link in the agent's final
